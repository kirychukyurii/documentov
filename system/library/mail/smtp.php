  ...
  public function send()
  {
    ...
    if (!$handle) {
      ...
      if (!empty($this->smtp_username)  && !empty($this->smtp_password)) {
        ...
        fputs($handle, 'HELO ' . getenv('HTTP_HOST') . "\r\n");
      ...
      if (substr($reply, 0, 3) != 250) {
        throw new \Exception('Error: DATA not accepted from server! ' . print_r($reply, true));
      }
