...
class ControllerInstallStep2 extends Controller
{
  ...
  public function index()
  {
    ...
    $data['text_exec'] = $this->language->get('text_exec');
    ...
    // catalog config
    if (!is_file(DIR_DOCUMENTOV . 'config.php')) {
    ...
    $data['exec'] = !in_array('exec', $disabled);
    $db = array(
      'mysql',
      'mysqli',
      'pgsql',
    ...
    $data['catalog_config'] = DIR_DOCUMENTOV . 'config.php';
    $data['image'] = DIR_DOCUMENTOV . 'image';
    $data['image_cache'] = DIR_DOCUMENTOV . 'image/cache';
    $data['image_catalog'] = DIR_DOCUMENTOV . 'image/catalog';
  ...
  private function validate()
  {
    ...
    if (!file_exists(DIR_DOCUMENTOV . 'config.php')) {
      $this->error['warning'] = $this->language->get('error_catalog_exist');
    } elseif (!is_writable(DIR_DOCUMENTOV . 'config.php')) {
