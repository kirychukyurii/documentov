...
define('DIR_DOCUMENTOV', str_replace('\\', '/', realpath(DIR_APPLICATION . '../')) . '/');
...
function check_requirements() {
	$error = null;
	if (phpversion() < '7.0') {
		$error = 'Warning: You need to use PHP7.0+ or above for OpenCart to work!';
	}
...
function write_config_files($options) {
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . addslashes(DIR_DOCUMENTOV) . 'catalog/\');' . "\n";
	...
	$output .= '// DIR' . "\n";
	$output .= 'define(\'DIR_APPLICATION\', \'' . addslashes(DIR_DOCUMENTOV) . 'admin/\');' . "\n";
	$output .= 'define(\'DIR_SYSTEM\', \'' . addslashes(DIR_DOCUMENTOV) . 'system/\');' . "\n";
	$output .= 'define(\'DIR_IMAGE\', \'' . addslashes(DIR_DOCUMENTOV) . 'image/\');' . "\n";	
	$output .= 'define(\'DIR_STORAGE\', DIR_SYSTEM . \'storage/\');' . "\n";
	$output .= 'define(\'DIR_CATALOG\', \'' . addslashes(DIR_DOCUMENTOV) . 'catalog/\');' . "\n";
	$output .= 'define(\'DIR_LANGUAGE\', DIR_APPLICATION . \'language/\');' . "\n";
	...
	$file = fopen(DIR_DOCUMENTOV . 'admin/config.php', 'w');
...
function dir_permissions() {
	$dirs = array(
		DIR_DOCUMENTOV . 'image/',
		DIR_DOCUMENTOV . 'system/storage/download/',
		DIR_DOCUMENTOV . 'system/storage/upload/',
		DIR_DOCUMENTOV . 'system/storage/cache/',
		DIR_DOCUMENTOV . 'system/storage/logs/',
		DIR_DOCUMENTOV . 'system/storage/modification/',
	);
...
case "install":
	try {
		...
		define('HTTP_DOCUMENTOV', $options['http_server']);
		$valid = valid($options);
		if (!$valid[0]) {
