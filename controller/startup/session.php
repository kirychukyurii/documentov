...
class ControllerStartupSession extends Controller
{
  public function index()
  {
    if (isset($this->request->get['route']) && substr($this->request->get['route'], 0, 4) == 'api/') {
      ...
      if (!$session_id && defined("DEV") && DEV && isset($_GET['session_id'])) {
        $session_id = $_GET['session_id'];
      }

      $this->session->start($session_id);
      //если ini_get('session.cookie_lifetime') > 0 - setcookie не работает если ini_get('session.cookie_lifetime') без time()+
      setcookie($this->config->get('session_name'), $this->session->getId(), (ini_get('session.cookie_lifetime') ? time() + ini_get('session.cookie_lifetime') : ini_get('session.cookie_lifetime')), ini_get('session.cookie_path'), ini_get('session.cookie_domain'));
    }
  }
}
